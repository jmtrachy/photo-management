...
async def test_get_album_returns_item_when_found():
    fake = {"album_id": "a1", "title": "Trip"}
    with patch.object(albums, "albums_table") as mock_table:
        mock_table.get_item.return_value = {"Item": fake}

        result = await albums.get_album("a1")

    assert result == fake
    mock_table.get_item.assert_called_once_with(Key={"album_id": "a1"})


async def test_get_album_returns_none_when_missing():
    with patch.object(albums, "albums_table") as mock_table:
...
async def test_create_album_puts_item():
...
async def test_list_recent_albums_queries_by_created_at():
...
async def test_list_recent_albums_empty():
    with patch.object(albums, "albums_table") as mock_table:
...
async def test_batch_get_albums_returns_mapping():
    with patch.object(albums, "albums_table") as mock_table, patch.object(
        albums, "dynamodb"
    ) as mock_dynamodb:
        mock_table.name = "test-albums-table"
...
async def test_batch_get_albums_empty_input():
    with patch.object(albums, "albums_table") as mock_table, patch.object(
        albums, "dynamodb"
    ) as mock_dynamodb:
        mock_table.name = "test-albums-table"
...
async def test_batch_get_albums_retries_unprocessed_keys():
    with patch.object(albums, "albums_table") as mock_table, patch.object(
        albums, "dynamodb"
    ) as mock_dynamodb:
        mock_table.name = "test-albums-table"
...
async def test_set_title_lowercases_search_field():
    with patch.object(albums, "albums_table") as mock_table:
...
async def test_remove_event_date():
    with patch.object(albums, "albums_table") as mock_table:
